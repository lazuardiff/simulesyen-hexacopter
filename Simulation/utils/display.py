...
def makeFigures(params, time, pos_all, vel_all, quat_all, omega_all, euler_all, commands, wMotor_all, thrust, torque, sDes_traj, sDes_calc):
    ...
    wM5 = wMotor_all[:, 4]*rads2rpm
    wM6 = wMotor_all[:, 5]*rads2rpm
    ...
    uM5 = commands[:, 4]*rads2rpm
    uM6 = commands[:, 5]*rads2rpm
    ...
    plt.grid(True)
    plt.legend(['roll', 'pitch', 'yaw', 'roll_sp',
               'pitch_sp', 'yaw_sp', 'yaw_tr'])
    plt.xlabel('Time (s)')
    ...
    plt.figure()
    plt.plot(time, wM1, time, wM2, time, wM3, time, wM4, time, wM5, time, wM6)
    plt.plot(time, uM1, '--', time, uM2, '--', time, uM3, '--',
             time, uM4, '--', time, uM5, '--', time, uM6, '--')
    plt.grid(True)
    ...
    plt.figure()
    plt.subplot(2, 1, 1)
    plt.plot(time, thrust[:, 0], time, thrust[:, 1], time, thrust[:, 2],
             time, thrust[:, 3], time, thrust[:, 4], time, thrust[:, 5])
    plt.grid(True)
    plt.legend(['thr1', 'thr2', 'thr3', 'thr4',
               'thr5', 'thr6'], loc='upper right')
    plt.xlabel('Time (s)')
    plt.ylabel('Rotor Thrust (N)')
    plt.draw()

    plt.subplot(2, 1, 2)
    plt.plot(time, torque[:, 0], time, torque[:, 1], time, torque[:, 2],
             time, torque[:, 3], time, torque[:, 4], time, torque[:, 5])
    plt.grid(True)
    plt.legend(['tor1', 'tor2', 'tor3', 'tor4',
               'tor5', 'tor6'], loc='upper right')
    plt.xlabel('Time (s)')
